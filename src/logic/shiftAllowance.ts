...
function resolveWorkAbsoluteSpan(
  ...
  let startMs = parseDateTimeMs(shiftDate, effectiveStart);
  let endMs = parseDateTimeMs(shiftDate, effectiveEnd);
  if (endMs <= startMs) endMs += DAY * 60 * 1000;
